<template>
  <div class="relative w-full max-w-3xl mx-auto">
    ...
    <div v-if="loading" class="absolute inset-0 flex items-center justify-center bg-black/50 z-50">
      ...
      <span class="text-white ml-4">Memuat kamera & model...</span>
    </div>

    <!-- Error Message -->
    <div v-if="errorMsg" class="absolute inset-0 flex items-center justify-center bg-black/80 z-50">
      <div class="bg-white text-red-600 p-4 rounded-lg shadow">
        {{ errorMsg }}
      </div>
    </div>
...
const gestureTimers = {} // Untuk auto-reset gesture
const gestureList = [
...
function showConfetti() {
  // Batasi hanya 1x per detik
  const now = Date.now()
  if (now - confettiLastTime < 1000) return
...
async function setupCamera() {
  try {
    ...
    if (!navigator.mediaDevices?.getUserMedia) throw new Error('Perangkat tidak mendukung webcam.')
    if (stream) stream.getTracks().forEach(t => t.stop())
    stream = await navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480 } })
    video.value.srcObject = stream
    cameraOn.value = true
  } catch (err) {
    errorMsg.value = 'Gagal mengakses kamera. Pastikan izin sudah diberikan.'
    loading.value = false
    throw err
  }
}
...
// Gesture detection
function setGesture(key, val, resetMs = 3000) {
  if (gestureState.value[key] === val) return
  gestureState.value[key] = val
  if (val) {
    clearTimeout(gestureTimers[key])
    gestureTimers[key] = setTimeout(() => {
      gestureState.value[key] = false
    }, resetMs)
  }
}
...
function detectGestures(results) {
  ...
  if (leftWrist && rightWrist && !gestureState.value.handUp) {
    if (leftWrist.y < 0.5 || rightWrist.y < 0.5) { setGesture('handUp', true); speak(myInfo); showConfetti() }
  }

  // Wave
  if (rightWrist) {
    const deltaX = rightWrist.x - (rightWrist.prevX || 0)
    if (!gestureState.value.wave && Math.abs(deltaX) > 0.03) { setGesture('wave', true); speak('Himatika! Kita pasti bisa!'); showConfetti() }
  ...
  if (nose) {
    ...
    if (!gestureState.value.nod && Math.abs(deltaY) > 0.03) { setGesture('nod', true); speak('Terima kasih, sampai jumpa!'); showConfetti() }
    nose.prevY = nose.y
  }
  ...
  if (hip) {
    ...
    if (!gestureState.value.jump && deltaY < -0.05) { setGesture('jump', true); speak('Lompat! Semangat!'); showConfetti() }
    hip.prevY = hip.y
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    // errorMsg sudah diisi di setupCamera
  }
})
...
onUnmounted(() => {
  ...
  // Bersihkan timer gesture
  Object.values(gestureTimers).forEach(timer => clearTimeout(timer))
})
</script>
...
.confetti { position: fixed; width: 8px; height: 8px; border-radius: 50%; animation: fall 1.8s linear forwards; pointer-events: none }
